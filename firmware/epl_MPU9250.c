...
void MPU9250_getData( uint16 *dataIMU ) //test failed ,wrong value of Magnetometer
{
   ...
   // WAIT_US(1000);
    ...
    dataIMU[1] = ((uint16)tmpRead[0] << 8 |  tmpRead[1]);  // Acc.X
    dataIMU[2] = ((uint16)tmpRead[2] << 8 |  tmpRead[3]);  // Acc.Y
    dataIMU[3] = ((uint16)tmpRead[4] << 8 |  tmpRead[5]);  // Acc.Z
    dataIMU[4] = ((uint16)tmpRead[8] << 8 |  tmpRead[9]);  // Gyr.X
    dataIMU[5] = ((uint16)tmpRead[10] << 8 |  tmpRead[11]);  // Gyr.Y
    dataIMU[6] = ((uint16)tmpRead[12] << 8 |  tmpRead[13]);  // Gyr.Z

//    if(!(tmpRead[14] & AK8963_STATUS_DRDY) || (tmpRead[14] & AK8963_STATUS_DOR) || (tmpRead[21] & AK8963_STATUS_HOFL))
//        return;

//    dataIMU[7] = ((uint16)tmpRead[15] << 8 |  tmpRead[14]);  // Mag.X
//    dataIMU[8] = ((uint16)tmpRead[17] << 8 |  tmpRead[16]);  // Mag.Y
//    dataIMU[9] = ((uint16)tmpRead[19] << 8 |  tmpRead[18]);  // Mag.Z
